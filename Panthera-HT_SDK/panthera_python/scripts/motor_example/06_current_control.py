...
if __name__ == "__main__":
    ...
    print(f"DQ 电流正弦控制: 振幅={AMPLITUDE} A, 频率={FREQUENCY} Hz")
    print("Ctrl+C 退出\n")
    ...
    try:
        while True:
            t = time.time()
            target_cur = AMPLITUDE * math.sin(2.0 * math.pi * FREQUENCY * t)

            for mid in MOTOR_IDS:
                state = ht.set_current(mid, target_cur)

                if cnt % PRINT_EVERY == 0 and state:
                    print(
                        f"电机 {mid}  "
                        f"目标电流: {target_cur:+.3f} A  "
