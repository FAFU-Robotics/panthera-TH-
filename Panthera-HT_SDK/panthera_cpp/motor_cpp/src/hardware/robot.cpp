...
namespace hightorque_robot
{
    namespace
    {
        bool has_vid_suffix(const std::string &hardware_id, const std::string &suffix)
        {
            std::string upper_hardware_id = hardware_id;
            for (char &c : upper_hardware_id)
            {
                c = static_cast<char>(std::toupper(static_cast<unsigned char>(c)));
            }

            std::string upper_suffix = suffix;
            for (char &c : upper_suffix)
            {
                c = static_cast<char>(std::toupper(static_cast<unsigned char>(c)));
            }

            return upper_hardware_id.find("VID_" + upper_suffix) != std::string::npos;
        }
    } // namespace
    ...
    int robot::serial_pid_vid(const char *name)
    {
        ...
        for (const auto &port : ports)
        {
            ...
            {
                return has_vid_suffix(port.hardware_id, "FAE1") ? 1 : -3;
            }

            return (has_vid_suffix(port.hardware_id, "CAF1") || has_vid_suffix(port.hardware_id, "CAE1") || has_vid_suffix(port.hardware_id, "0483")) ? 1 : -3;
        }
        return -1;
#else
        int pid, vid;
    ...
    std::vector<std::string> robot::list_serial_ports(const std::string& full_prefix) 
    {
        ...
        for (const auto &port : ports)
        {
            if (port.port.find(full_prefix) == 0)
            {
                serial_ports.push_back(port.port);
            }            
        }
    ...
    int robot::check_serial_dev_exist(int file_num)
    {
        ...
        {
            std::string _dev = prefix + std::to_string(i);
