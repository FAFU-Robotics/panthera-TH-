...
按顺序跑下面 5 个检查, 哪一步先红就先修哪一步:
    ...
    3. import wrs_panthera_arm_controller 成功
    4. robot.cfg 找得到 + 能解析
    5. find_likely_debug_boards() 列出 USB 调试板 (机械臂可不上电)
...
def step_import_controller() -> int:
    section("3. import wrs_panthera_arm_controller")
    try:
        from wrs_panthera_arm_controller import PantheraArmController
    except Exception as e:
        traceback.print_exc()
        return failed(f"import wrs_panthera_arm_controller 失败: {e}")
    passed("import OK")
    ...
    miss = [m for m in expected_methods if not hasattr(PantheraArmController, m)]
    if miss:
        return failed(f"PantheraArmController 缺少方法: {miss}")
    passed(f"PantheraArmController 接口完整 ({len(expected_methods)} 个方法)")

    rad = PantheraArmController._rad_to_turns(3.141592653589793)
    if abs(rad - 0.5) > 1e-9:
        return failed(f"_rad_to_turns(pi) 应该 = 0.5, 得到 {rad}")
    rad2 = PantheraArmController._turns_to_rad(0.5)
...
def step_cfg_load(cfg_path: str) -> int:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="wrs_panthera_arm_controller smoke test")
    parser.add_argument("--cfg", default="robot.cfg",
                        help="robot.cfg 路径 (默认: panthera_web/robot.cfg)")
    args = parser.parse_args()

    print("=" * 60)
    print("  wrs_panthera_arm_controller smoke test")
    print("=" * 60)
