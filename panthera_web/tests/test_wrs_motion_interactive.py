# -*- coding: utf-8 -*-
"""
test_wrs_motion_interactive.py
==============================

带菜单的交互式实验脚本 (wrs 风格的高层封装 wrs_panthera_arm_controller).
不用反复改代码就能跑遍 PantheraArmController 的各种功能
...
用法:
    cd panthera_web
    python tests/test_wrs_motion_interactive.py                      # 默认无夹爪
    python tests/test_wrs_motion_interactive.py --gripper-id 7       # 7 号是夹爪
    python tests/test_wrs_motion_interactive.py --speed 10           # 全局降速
"""
...
class App:
    def __init__(self, arm, default_speed: int):
        ...
        assert isinstance(arm, PantheraArmController)
        self.arm = arm
...
def main() -> int:
    ...
    try:
        ...
        traceback.print_exc()
        print(f"\n[FAIL] import wrs_panthera_arm_controller 失败: {e}")
    ...
    try:
        ...
        traceback.print_exc()
        print(f"\n[FAIL] 创建 PantheraArmController 失败: {e}")
        return 1
