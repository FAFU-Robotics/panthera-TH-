...
│
├── fafu_robot_cpp/         ← C++ SDK (★ 完全自包含 ★)
│   ├── README.md
│   ├── bindings.cpp        pybind11 绑定源码
│   ├── CMakeLists.txt      构建配置
...
| 侧 | 角色 | 输出 | 用户是否需要碰 |
|---|---|---|---|
| **`fafu_robot_cpp/`** | 调试板 USB 串口协议 + pybind11 绑定 (所有源码已 vendor, 无需外部 `Panthera-HT_SDK/`) | `panthera_motor.cpXY-win_amd64.pyd` + `serial_cmake.dll` | 一般只在 **第一次部署 / 改 binding / 切换 Python 版本** 时需要构建一次 |
...
- **C++ 侧 (构建 / 改 binding)** → 看 [`fafu_robot_cpp/README.md`](fafu_robot_cpp/README.md)
